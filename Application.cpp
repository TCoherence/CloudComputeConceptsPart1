...
void handler(int sig) {
	// comment the following 2 lines because we are on Windows platform
	void *array[10];
	size_t size;

	// get void*'s for all entries on the stack, comment this since we are running this on windows...
	size = backtrace(array, 10);

	// print out all the frames to stderr
	fprintf(stderr, "Error: signal %d:\n", sig);
	backtrace_symbols_fd(array, size, STDERR_FILENO); // comment this because we are on windows.
	exit(1);
}
